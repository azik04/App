...
public class GetAllByWorkerQueryHandler : IRequestHandler<GetAllByWorkerQuery, GenericResponse<List<GetAllJobDto>>>
{
    ...
    public GetAllByWorkerQueryHandler(IGenericRepository<Jobs> jobRepository) => _jobRepository = jobRepository;
    ...
    {
        var data = await _jobRepository.Where(x => x.ServiceId == request.serviceId && x.StatusId == 1)
            .Include(x => x.Worker)
            .Include(x => x.Client)
            .Include(x => x.Address)
            .Include(x => x.Statuse)
